...
class Post(models.Model):
    ...
    def parser(self):
        ...
        for item in content_header:
            ...
            try:
                ...
                print(f"Ошибка парсера: ссылка на видео не найдена - {e}")
    ...
    def check_channel(self, channel, channel_en):
        if channel is None:
            ...
            if channel.name_en is None:
                channel.name_en = channel_en
                channel.save()
                return channel
            return channel
        ...
        if tags is None:
            ...
            for tag in tags:
                if Tag.objects.get_or_create(name=tag)[1]:
                    tag_obj = Tag.objects.get_or_create(name=tag, name_en=tags_en[count])[0]
                    tag_obj.save()
                    tags_obj.append(tag_obj)
                else:
                    tag_obj = Tag.objects.get_or_create(name=tag)[0]
                    if tag_obj.name_en is None:
                        tag_obj.name_en = tags_en[count]
                        tag_obj.save()
                        tags_obj.append(tag_obj)
                    tags_obj.append(tag_obj)
                count = count + 1
    ...
    def add_tags(self, tags_ru, tags_en):
        for tag in self.check_tags(tags=tags_ru, tags_en=tags_en):
            self.tags.add(tag) 
    ...
    def publish_post(self):
        img_1_href, channel_en, title_en, text_en, video_1_data_scr, tags_en, html = self.parser()
        ...
        self.author = [user for user in User.objects.all() if user.username == "root"][0]
        ...
        self.save()
        self.create_image(href=img_1_href)
        self.add_tags(tags_ru=tags_ru, tags_en=tags_en)
    ...
    def clean(self) -> None:
        ...
        try:
            ...
            if self.created_date is not None:
                ...
                raise ValidationError(f"{self.created_date} field is not Validate")

            if self.pub is not None:
                print(f"{self.pub} is pub field")
            else:
                raise ValidationError(f"{self.pub} field is not Validate")
